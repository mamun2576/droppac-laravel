<div class="w-full">
    <form class="mt-2" wire:submit.prevent="createService">
        @csrf
        <div class="flex flex-col">
            <div class="my-6">
                <h2 class="text-lg font-medium text-purple">Create a new domestic delivery service</h2>
                <p class="text-xs text-gray-400">Complete and submit the form below to create a new delivery service.</p>
            </div>
            <div class="flex flex-col sm:flex-row sm:space-x-8">
                ...
                <div class="w-full">
                    <x-jet-label for="delivery_time" value="{{ __('Expected hours to delivery (hrs)') }}" />
                    <x-jet-input  id="delivery_time" name="delivery_time" type="number" class="mt-1 block w-full" wire:model.defer="delivery_time" autocomplete="delivery_time"/>
                    <x-jet-input-error for="delivery_time" class="mt-2" />
                </div>
                <div class="w-full">
   ...
   <div class="mt-8">
        <div class="my-6">
            <h2 class="text-lg font-medium text-purple">All delivery services</h2>
        </div>
            ...
            @foreach($services as $service)
                <tr>
                  ...
                  <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    <a href="">{{ $service->delivery_time }} </a>
                  </td> 
                  <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    <p>{{ $service->cost }} </p>
                  </td>
